...
class FunSetSuite extends FunSuite {
  ...
  test("forall") {
    new TestSets {
      val u1 = union(s1, s2)
      val u2 = union(u1, s3)
      assert(forall(u1, List(1,2).contains(_)), "Forall 1")
      assert(!forall(u2, List(1,2).contains(_)), "Forall 1")
    }
  }
  ...
  test("exists") {
    new TestSets {
      val u1 = union(s1, s2)
      val u2 = union(u1, s3)
      assert(exists(u1, _ == 1), "Exists 1")
      assert(exists(u1, _ == 2), "Exists 2")
      assert(!exists(u1, _ == 3), "Exists 3")
    }
  }
