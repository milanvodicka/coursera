...
trait Solver extends GameDef {
  ...
  def from(initial: Stream[(Block, List[Move])], explored: Set[Block]): Stream[(Block, List[Move])] = {
    def next(initial: Stream[(Block, List[Move])], explored: Set[Block]): Stream[(Block, List[Move])] = {
      val neighbors = for {
        (block, moves) <- initial
      ...
      if (neighbors.isEmpty) Stream.empty
        ...
        next(neighbors, explored ++ neighbors.toSet.map((p:(Block, List[Move])) => p match { case (block, _) => block }))
    }
    initial #::: next(initial, explored)
  }
